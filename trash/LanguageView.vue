...
const programingLanguagesFields = ref([]);
const showModal = ref(false);
const submissionToEdit = ref({});
const choices = ref({});

async function fetchprogramingLanguages() {
  try {
    const r = await axios.get("/api/programingLanguages/");
    programingLanguages.value = r.data;
    console.log(r.data)
    const f = await axios.get("/api/programingLanguages/fields/");
    programingLanguagesFields.value = f.data;

    programingLanguagesFields.value.forEach((field) => {
...
async function onRemoveClick(student) {
  try {
    await axios.delete(`/api/programingLanguages/${student.id}/`);
    await fetchprogramingLanguages();
  } catch (error) {
    console.log(error);
  }
}
...
async function onLessonNameAddClick() {
  ...
  await axios.post("/api/programingLanguages/", {
...
async function onLessonNameEditClick() {
  try {
    ...
    await axios.put(`/api/programingLanguages/${submissionToEdit.value.id}/`, {
...
<template>
  <div>
        ...
        <div class="row">
          <div class="col">
            <div class="form-floating">
              <div
                v-for="item in programingLanguagesFields"
                :key="item.id"
                class="student-item"
              >
    ...
    <div class="edit-frame">
      <div v-for="item in programingLanguages" :key="item.id" class="student-item">
        <div>{{ item.name }}</div>
        <button
